...
public:
    ...
    BmpImage(unsigned int w, unsigned int h);

    BmpImage(const sensor_msgs::Image::ConstPtr& img);
    ...
    BmpImage<T>* shrinkImage(unsigned int times);
...
template <class T>
BmpImage<T>::BmpImage()
: m_imageMatrix(NULL), m_width(0), m_height(0)
{
}

template <class T>
BmpImage<T>::BmpImage(unsigned int w, unsigned int h)
...
template <class T>
BmpImage<T>::BmpImage(const sensor_msgs::Image::ConstPtr& img)
{
...
void BmpImage<T>::writeToMessage(const sensor_msgs::Image::ConstPtr& img)
{
    ...
    {                        
        ...
        /*
        int all = img->width * img->height * 3;
        for(int i = 0; i < all; i++) {
            unsigned char* g = (unsigned char*)&img->data[i];
            *g = rand() % 255;
        }
        */ 
    }
...
template <class T>
BmpImage<T>* BmpImage<T>::shrinkImage(unsigned int times)
{
    unsigned int width = m_width / times;
    unsigned int height = m_height / times;

    BmpImage<T>* shrinkedImage = new BmpImage<T>(width, height);
    Pixel<T>*** matrix = shrinkedImage->getImageMatrix();
    ...
    for (i = 0, in = 0; i < m_height && in < height; i += times, in++)
    {
        ...
        {                        
            matrix[in][jn] = m_imageMatrix[i][j];            
        }
    }        
